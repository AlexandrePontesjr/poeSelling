  ...
  return (
    <section id="clients" className="w-full py-12 md:py-24 lg:py-32">
      ...
      <div className="mt-4 flex justify-center">
          <button onClick={handleModalToggle} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Enviar Feedback
          </button>
        </div>
        {feedbackSent && (
          <div className={` items-center justify-center text-center text-white bg-green-400/50 py-2 px-4 rounded mt-4`}>
            <span>
        ...
        <Modal isOpen={modalOpen} onClose={handleModalToggle} game={game}>
          <div className="">
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                <label htmlFor="avatar" className="block text-sm font-medium text-gray-900">Avatar:</label>
                <SelectAvatar value={avatar} onChange={(e) => setAvatar(e.target.value)} />
              </div>
              <div className="mb-4">
                <label htmlFor="name" className="block text-sm font-medium text-gray-900">Nome:</label>
                <input type="text" id="name" name="name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
              </div>
              <div className="mb-4">
                <label htmlFor="rating" className="block text-sm font-medium text-gray-900">Classificação:</label>
                <Rating
              ...
              <div className="mb-4">
                ...
                <textarea id="message" name="message" value={message} onChange={(e) => setMessage(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"></textarea>
              </div>
              <div className="mt-4">
                <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Enviar</button>
                <button onClick={handleModalToggle} className="ml-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">Cancelar</button>
              </div>
              {errorMessage && (
